...
build/
  ...
  scripts/
    common/               <- reusable extraction + assembly tools
                             (incl. build_sitemap.py -> repo-root sitemap.xml + robots.txt)
    pages/                <- one (or two) build scripts per page
    build_all.py           <- regenerates all 13 pages + sitemap.xml + robots.txt in one run
...
This overwrites the repo-root `.html` files directly (the live pages), plus `sitemap.xml`
and `robots.txt` (both generated by `scripts/common/build_sitemap.py` since Milestone 2.4 —
the sitemap's 13-URL canonical map lives in that script's `CANONICAL_URLS` list; edit it
there, never hand-edit sitemap.xml). **Always run `git
