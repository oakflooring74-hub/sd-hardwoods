...
2. **Internal link from blog page:** Edit `build/scripts/pages/assemble_blog.py` → append a dict to `PROJECT_SHOWCASE_CARDS`. This single list drives ALL three blog integrations: the "Individual Project Pages" hub `<ul>` links, case study cards with in-card "Full project page:" internal links, and BlogPosting JSON-LD schema entries.
...
> **CRITICAL NOTE (2026-09):** `case_studies.json`'s `featured_projects` array is NOT read by the blog builder. The single source of truth for all blog integration is `PROJECT_SHOWCASE_CARDS` in `assemble_blog.py`. Adding an entry there automatically populates hub links, case study cards with internal page links, and BlogPosting schema.
...
4. **Enhance owner dictation with natural-language SEO phrases and CTAs.** The agent takes the baseline facts from step 1 and weaves them into conversational search terms that real people type (e.g., "vintage floor refinishing in La Jolla," "red oak restoration San Diego") plus a clear call-to-action ("free phone and photo assessment"). This is done automatically — no need to ask. The goal: every project page becomes an SEO monster by default, pulling its weight for long-tail keyword rankings.
...
Edit `build/scripts/pages/assemble_blog.py` → append a dict to `PROJECT_SHOWCASE_CARDS`:
```python
{
    "id": "module_itemRecordGuid.[project_slug]_2026_09",
    "title": "# XX [Short Title] — Longer Description",
    "page_url": "https://www.sdhardwoods.com/[seo-heavy-url].html",
    "page_label": "Keyword-rich anchor text for links (unique per page)",
    "hub_blurb": "One-sentence description shown only in hub section list.",
    "prose": [
        "Real project copy — SEO-enhanced owner dictation with natural keyword mentions."
    ],
    "images": [
        {
            "href": "/[image-file-name].png",
            "src": "/[image-file-name].png",
            "alt": "Image-specific alt text prefix (title + prose appended by build)",
            "class": "",
        },
    ],
    "captions": [],
...
This single dict populates: the hub section `<ul>` link, the case study card with two images and an in-card internal link to the full project page, and a `BlogPosting` JSON-LD schema entry.
...
| `build/scripts/pages/assemble_blog.py` → `PROJECT_SHOWCASE_CARDS` | Blog hub links + case study cards + BlogPosting schema | Append one dict entry per new page — **this is the single source of truth for blog integration** |
| `build/data/blog/case_studies.json` → `featured_projects` | Reference data (future-proofing) | Add entry here too, but note: assemble_blog.py does NOT read this field — it's for documentation/re-generation safety |
| `build/scripts/common/public_business_rules.py` → `BREADCRUMB_NAMES` | Breadcrumb schema mapping | Add canonical URL + display name per new page |
...
| 14 | Breadcrumb mapping added | Search `public_business_rules.py` BREADCRUMB_NAMES for new canonical URL |
| 15 | Blog hub link + case study card | Check `assemble_blog.py` → `PROJECT_SHOWCASE_CARDS` has a dict entry for the page (drives hub links, in-card "Full project page:" link, and BlogPosting schema) |

**If all checks pass:** Task complete. The individual project showcase page has been successfully created, deployed to production, and is indexed by Google via sitemap for SEO footprint expansion.
...
> **"Add [project name] individual showcase page from folder [folder path]. All images ready with SEO filenames. Follow established workflow: build script → sitemap (14+ URLs) → redirect rules → breadcrumb mapping in public_business_rules.py → blog link via assemble_blog.py PROJECT_SHOWCASE_CARDS (#XX) → commit → push to redesign for preview deployment."**

**Example:**
> *"Add Muirlands Oak individual showcase page from folder MUIRLANDS OAK FLOOR REFINISHING LA JOLLA SAN DIEGO WATERMARKED. All 28 images ready with SEO filenames. Follow established workflow: build script → sitemap (14 URLs) → redirect rules → breadcrumb mapping in public_business_rules.py → blog link via assemble_blog.py PROJECT_SHOWCASE_CARDS (#13) → commit → push to redesign for preview deployment."*
